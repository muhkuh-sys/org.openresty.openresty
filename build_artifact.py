...
if tPlatform['host_distribution_id'] == 'ubuntu':
    if tPlatform['distribution_id'] == 'ubuntu':
        ...
        if tPlatform['cpu_architecture'] == tPlatform['host_cpu_architecture']:
            ...
            # This is terrible, but there was no way to pass the include and library paths as parameters to the openresty
            # build scripts.
            if os.path.exists('/usr/riscv64-linux-gnu/lib/libcrypt.a') is not True:
                os.symlink(
                    os.path.join(
                        strCfg_workingFolder,
                        'packages/usr/lib/riscv64-linux-gnu/libcrypt.a'
                    ),
                    '/usr/riscv64-linux-gnu/lib/libcrypt.a'
                )
            if os.path.exists('/usr/riscv64-linux-gnu/include/crypt.h') is not True:
                os.symlink(
                    os.path.join(
                        strCfg_workingFolder,
                        'packages/usr/include/crypt.h'
                    ),
                    '/usr/riscv64-linux-gnu/include/crypt.h'
                )
